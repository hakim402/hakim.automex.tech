...
export async function generateStaticParams() {
  const slugs = getContentSlugs("achievements");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const item = getContentBySlug("achievements", slug);
  if (!item) return { title: "Not Found" };

  const title = `${item.meta.title} | Achievement`;
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${SITE.url}/achievements/${slug}`,
    },
    twitter: {
      card: "summary",
      title,
    ...
    alternates: {
      canonical: item.meta.canonicalUrl || `${SITE.url}/achievements/${slug}`,
    },
  };
}
