...
export async function generateStaticParams() {
  const slugs = getContentSlugs("publications");
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${item.meta.title} | Publication`;
  ...
  return {
    ...
    alternates: {
      canonical:
        item.meta.canonicalUrl || `${SITE.url}/publications/${slug}`,
    },
  };
}
...
export default async function PublicationDetail({ params }: Props) {
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6 sm:py-20">
      ...
      {/* Trace-rule divider */}
      <div className="trace-rule mx-auto mb-10 h-px w-full" />
